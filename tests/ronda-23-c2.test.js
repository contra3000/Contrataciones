...
// ORDEN-RONDA-23 §3: el presupuesto viaja como bytes crudos; el tipo va en la
// cabecera y el nombre original en X-SGC-Nombre-Original. La sesión (cookie)
// reemplaza al contexto, igual que en el resto de la API autenticada.
function subirPresupuesto(base, id, cookie, nombre) {
  return su.enviarBytes(base, '/api/expedientes/' + id + '/presupuestos',
    Buffer.from('%PDF-1.4 presupuesto de la ronda 23'),
    {
      'Content-Type': 'application/pdf',
      'X-SGC-Nombre-Original': encodeURIComponent(nombre || 'proveedor.pdf'),
      Cookie: cookie
    });
}
...
test('presupuestos: guarda el rol que ejecuta el estado y rechaza a los demás', async () => {
  ...
  try {
    ...
    const ajeno = await subirPresupuesto(e.base, id, e.cookies.abastecimiento);
    ...
    const propio = await subirPresupuesto(e.base, id, e.cookies.generador);
...
test('las tres guardias contra los siete roles: pasa el que corresponde, los otros seis 403', async () => {
  ...
  try {
    ...
    for (const rol of ROLES7) {
      const r = await subirPresupuesto(e.base, idPresu, e.cookies[rol]);
